( function( $, wp ) {
	...
	wp.updates.importDemo = function( args ) {
		...
		if ( $message.html() !== __( 'Importing...', 'indi-demo-importer' ) ) {
			$message.data( 'originaltext', $message.html() );
		}

		$message
			.text( __( 'Importing...', 'indi-demo-importer' ) )
			.attr(
				'aria-label',
				sprintf(
					/* translators: %s: Demo name. */
					_x( 'Importing %s...', 'demo', 'indi-demo-importer' ),
					$message.data( 'name' )
				)
			);
		wp.a11y.speak( __( 'Importing... please wait.', 'indi-demo-importer' ), 'polite' );
	...
	wp.updates.importDemoSuccess = function( response ) {
		...
		$message = $card.find( '.button-primary' )
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Demo name. */
					_x( '%s imported!', 'demo', 'indi-demo-importer' ),
					response.demoName
				)
			)
			.text( __( 'Imported!', 'indi-demo-importer' ) );

		wp.a11y.speak( __( 'Import completed successfully.', 'indi-demo-importer' ), 'polite' );
			...
			if ( response.previewUrl ) {
				...
				$message
					...
					.attr(
						...
						sprintf(
							/* translators: %s: Demo name. */
							_x( 'Live Preview %s', 'demo', 'indi-demo-importer' ),
							response.demoName
						)
					)
					.text( __( 'Live Preview', 'indi-demo-importer' ) );
			}
		}, 1000 );
	};
	...
	wp.updates.importDemoError = function( response ) {
		...
		var $card, $button,
			errorMessage = sprintf(
				...
				__( 'Import failed: %s', 'indi-demo-importer' ),
		...
		$button
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Demo name. */
					_x( '%s import failed', 'demo', 'indi-demo-importer' ),
					$button.data( 'name' )
				)
			)
			.text( __( 'Import Failed!', 'indi-demo-importer' ) );
	...
	wp.updates.bulkInstallPlugin = function( args ) {
		...
		if ( $document.find( 'body' ).hasClass( 'full-overlay-active' ) ) {
			...
			message    = sprintf(
				...
				_x( 'Installing %s...', 'plugin', 'indi-demo-importer' ),
				$pluginRow.data( 'name' )
			);
			$pluginRow.find( '.plugin-status span' )
				...
				.text( __( 'Installing...', 'indi-demo-importer' ) );
		} else {
		...
		if ( $message.html() !== __( 'Installing...', 'indi-demo-importer' ) ) {
			$message.data( 'originaltext', $message.html() );
		}
		...
		wp.a11y.speak( __( 'Installing... please wait.', 'indi-demo-importer' ), 'polite' );
	...
	wp.updates.bulkInstallPluginSuccess = function( response ) {
		...
		$updateMessage
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Plugin name. */
					_x( '%s installed!', 'plugin', 'indi-demo-importer' ),
					response.pluginName
				)
			)
			.text( _x( 'Installed!', 'plugin', 'indi-demo-importer' ) );

		wp.a11y.speak( __( 'Installation completed successfully.', 'indi-demo-importer' ), 'polite' );
	...
	wp.updates.bulkInstallPluginError = function( response ) {
		...
		errorMessage = sprintf(
			...
			__( 'Installation failed: %s', 'indi-demo-importer' ),
			response.errorMessage
		);
		...
		$updateMessage
			...
			.attr(
				...
				sprintf(
					/* translators: %s: Plugin name. */
					_x( '%s installation failed', 'plugin', 'indi-demo-importer' ),
					$pluginRow.data( 'name' )
				)
			)
			.text( __( 'Installation Failed!', 'indi-demo-importer' ) );
	...
	wp.updates.isValidResponse = function( response, action ) {
		console.log( response )
		var error = __( 'Something went wrong.', 'indi-demo-importer' ),
			errorMessage;
		...
		if ( _.isString( response ) && '-1' === response ) {
			error = __( 'An error has occurred. Please reload the page and try again.', 'indi-demo-importer' );
		} else if ( _.isString( response ) ) {
			error = response;
		} else if ( 'undefined' !== typeof response.readyState && 0 === response.readyState ) {
			error = __( 'Connection lost or the server is busy. Please try again later.', 'indi-demo-importer' );
		} else if ( _.isString( response.statusText ) ) {
			error = response.statusText + ' ' + '<a href="https://docs.indithemes.com/knowledgebase/demo-import-process-failed/" target="_blank">' + __( 'Try this solution!', 'indi-demo-importer' ) + '</a>';
		}
o
		switch ( action ) {
			case 'import':
				errorMessage = __( 'Import failed: %s', 'indi-demo-importer' );
				break;

			case 'install':
				errorMessage = __( 'Installation failed: %s', 'indi-demo-importer' );
				break;
		}
